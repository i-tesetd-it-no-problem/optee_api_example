...
static void generate_key(struct aes_ccm_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, AES_CCM_GEN_KEY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void encrypt(struct aes_ccm_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, AES_CCM_AE_ENCRYPR, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct aes_ccm_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
...
static void prepare_tee_session(struct aes_ccm_ctx *ctx)
{
	TEEC_UUID uuid = TA_AES_CCM_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct aes_ccm_ctx *ctx)
{
	generate_key(ctx);
	encrypt(ctx);
	decrypt(ctx);
...
static void terminate_tee_session(struct aes_ccm_ctx *ctx)
{
 ...
 * scp aes_ccm/ta/5f7df382-c9b6-496b-8979-67f13ce9685f.ta wenshuyu@192.168.1.6:/lib/optee_armtz
 * scp aes_ccm/host/aes_ccm wenshuyu@192.168.1.6:/usr/bin
 */
