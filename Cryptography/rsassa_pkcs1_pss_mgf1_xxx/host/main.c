...
#include "../ta/include/rsassa_pkcs1_pss_mgf1_xxx.h"
...
static void generate_key(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
	TEEC_Operation op;
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSASSA_PKCS1_PSS_MGF1_XXX_GEN_KEY, &op, &error_origin);
	if(res != TEEC_SUCCESS) 
...
static void digest(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
	TEEC_Operation op;
	uint32_t error_origin;
	TEEC_Result res;
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSASSA_PKCS1_PSS_MGF1_XXX_DIGEST, &op, &error_origin);
	if(res != TEEC_SUCCESS) 
...
static void sign(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
	TEEC_Operation op;
	uint32_t error_origin;
	TEEC_Result res;
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSASSA_PKCS1_PSS_MGF1_XXX_SIGN, &op, &error_origin);
	if(res != TEEC_SUCCESS) 
		errx(1, "sign failed\n");
...
static void verify(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
	TEEC_Operation op;
	uint32_t error_origin;
	TEEC_Result res;
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSASSA_PKCS1_PSS_MGF1_XXX_VERIFY, &op, &error_origin);
	if(res != TEEC_SUCCESS) 
		errx(1, "verify failed\n");
}

static void example(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
	generate_key(ctx);
	digest(ctx);
	sign(ctx);
...
static void prepare_tee_session(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
	TEEC_UUID uuid = TA_RSASSA_PKCS1_PSS_MGF1_XXX_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void terminate_tee_session(struct rsassa_pkcs1_pss_mgf1_xxx_ctx *ctx)
{
...
int main()
{
    struct rsassa_pkcs1_pss_mgf1_xxx_ctx ctx;
