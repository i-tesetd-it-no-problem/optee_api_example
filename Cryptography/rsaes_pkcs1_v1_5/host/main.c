...
#include "../ta/include/rsaes_pkcs1_v1_5.h"
...
static void generate_key(struct rsaes_pkcs1_v1_5_ctx *ctx)
{
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSAES_PKCS1_V1_5_GEN_KEY, &op, &error_origin);
	if(res != TEEC_SUCCESS) 
...
static void encrypt(struct rsaes_pkcs1_v1_5_ctx *ctx)
{
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSAES_PKCS1_V1_5_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct rsaes_pkcs1_v1_5_ctx *ctx)
{
	...
	res = TEEC_InvokeCommand(&ctx->sess, RSAES_PKCS1_V1_5_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
{
	TEEC_UUID uuid = TA_RSAES_PKCS1_V1_5_UUID;
	uint32_t origin;
	TEEC_Result res;
