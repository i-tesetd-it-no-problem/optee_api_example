...
#include "../ta/include/aes_ecb_nopad.h"
...
struct aes_ecb_nopad_ctx {
	TEEC_Context ctx;
	TEEC_Session sess;
};

static void generate_key(struct aes_ecb_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
...
static void encrypt(struct aes_ecb_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_ECB_NOPAD_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct aes_ecb_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_ECB_NOPAD_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void prepare_tee_session(struct aes_ecb_nopad_ctx *ctx)
{
	TEEC_UUID uuid = TA_AES_ECB_NOPAD_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct aes_ecb_nopad_ctx *ctx)
{
	generate_key(ctx);
	encrypt(ctx);
	decrypt(ctx);
...
static void terminate_tee_session(struct aes_ecb_nopad_ctx *ctx)
{
 ...
 * scp aes_ecb_nopad/ta/2f1c50d5-7b64-4f11-a4cf-8e1ad86fb560.ta wenshuyu@192.168.1.6:/lib/optee_armtz
 * scp aes_ecb_nopad/host/aes_ecb_nopad wenshuyu@192.168.1.6:/usr/bin
 */
