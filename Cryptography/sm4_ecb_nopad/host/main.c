...
#include "../ta/include/sm4_ecb_nopad.h"
...
struct sm4_ecb_nopad_ctx {
	TEEC_Context ctx;
	TEEC_Session sess;
};

static void generate_key(struct sm4_ecb_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
...
static void encrypt(struct sm4_ecb_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_ECB_NOPAD_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct sm4_ecb_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_ECB_NOPAD_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void prepare_tee_session(struct sm4_ecb_nopad_ctx *ctx)
{
	TEEC_UUID uuid = TA_SM4_ECB_NOPAD_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct sm4_ecb_nopad_ctx *ctx)
{
	generate_key(ctx);
	encrypt(ctx);
	decrypt(ctx);
...
static void terminate_tee_session(struct sm4_ecb_nopad_ctx *ctx)
{
