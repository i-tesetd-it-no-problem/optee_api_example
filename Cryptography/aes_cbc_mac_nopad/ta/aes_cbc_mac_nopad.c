...
#include "include/aes_cbc_mac_nopad.h"
...
static TEE_Result generate_key(void *sess_ctx, uint32_t param_type, TEE_Param params[4])
{
    ...
    struct aes_cbc_mac_nopad *ctx = (struct aes_cbc_mac_nopad *)sess_ctx;
    ...
    res = TEE_IsAlgorithmSupported(TEE_ALG_AES_CBC_MAC_NOPAD, TEE_CRYPTO_ELEMENT_NONE);
    if (res != TEE_SUCCESS) {
    ...
    res = TEE_AllocateOperation(&ctx->operation, TEE_ALG_AES_CBC_MAC_NOPAD, TEE_MODE_MAC, KEY_BITS);
    if (res != TEE_SUCCESS) {
...
static TEE_Result do_mac(void *sess_ctx, uint32_t param_type, TEE_Param params[4])
{
    ...
    if (IS_NOT_MULTIPLE_OF_BLOCK_SIZE(params[0].memref.size)) {
        EMSG("Input data length must be a multiple of 16 bytes\n");
        return TEE_ERROR_BAD_PARAMETERS;
    }
...
TEE_Result TA_OpenSessionEntryPoint(uint32_t param_type, TEE_Param params[4], void **sess_ctx)
{
    ...
    struct aes_cbc_mac_nopad *ctx = TEE_Malloc(sizeof(struct aes_cbc_mac_nopad), TEE_MALLOC_FILL_ZERO);
    if (!ctx) {
...
{
    switch (cmd) {
    case AES_CBC_MAC_NOPAD_GEN_KEY:
        return generate_key(sess_ctx, param_type, params);

    case AES_CBC_MAC_NOPAD_GEN_MAC:
        return do_mac(sess_ctx, param_type, params);

    case AES_CBC_MAC_NOPAD_VERIFY:
