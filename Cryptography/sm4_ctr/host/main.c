...
static void generate_key(struct sm4_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CTR_GEN_KEY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void generate_iv(struct sm4_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CTR_GEN_IV, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void encrypt(struct sm4_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CTR_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct sm4_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CTR_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void prepare_tee_session(struct sm4_ctr_ctx *ctx)
{
	TEEC_UUID uuid = TA_SM4_CTR_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct sm4_ctr_ctx *ctx)
{
	generate_key(ctx);
	generate_iv(ctx);
	encrypt(ctx);
...
static void terminate_tee_session(struct sm4_ctr_ctx *ctx)
{
 ...
 * scp sm4_ctr/ta/6ac5a9a5-d5fc-4d0e-b8c3-7f76deb015fb.ta wenshuyu@192.168.1.6:/lib/optee_armtz
 * scp sm4_ctr/host/sm4_ctr wenshuyu@192.168.1.6:/usr/bin
 */
