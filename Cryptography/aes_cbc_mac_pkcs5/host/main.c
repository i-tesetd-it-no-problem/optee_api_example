...
#include "../ta/include/aes_cbc_mac_pkcs5.h"
...
struct aes_cbc_mac_pkcs5_ctx {
	TEEC_Context ctx;
	TEEC_Session sess;
	uint8_t mac[MAC_LEN];
};

static void generate_key(struct aes_cbc_mac_pkcs5_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, AES_CBC_MAC_PKCS5_GEN_KEY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void do_mac(struct aes_cbc_mac_pkcs5_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, AES_CBC_MAC_PKCS5_GEN_MAC, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void verify(struct aes_cbc_mac_pkcs5_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, AES_CBC_MAC_PKCS5_VERIFY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
		errx(1, "verify failed\n");
	}
}

static void prepare_tee_session(struct aes_cbc_mac_pkcs5_ctx *ctx)
{
	TEEC_UUID uuid = TA_AES_CBC_MAC_PKCS5_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct aes_cbc_mac_pkcs5_ctx *ctx)
{
	generate_key(ctx);
	do_mac(ctx);
	verify(ctx);
...
static void terminate_tee_session(struct aes_cbc_mac_pkcs5_ctx *ctx)
{
...
int main()
{
    struct aes_cbc_mac_pkcs5_ctx ctx;
