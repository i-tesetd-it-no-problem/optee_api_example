...
#include "../ta/include/sm4_cbc_nopad.h"
...
struct sm4_cbc_nopad_ctx {
	TEEC_Context ctx;
	TEEC_Session sess;
	uint8_t iv[IV_SIZE];
};

static void generate_key(struct sm4_cbc_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CBC_NOPAD_GEN_KEY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void generate_iv(struct sm4_cbc_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CBC_NOPAD_GEN_IV, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void encrypt(struct sm4_cbc_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CBC_NOPAD_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct sm4_cbc_nopad_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_SM4_CBC_NOPAD_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void prepare_tee_session(struct sm4_cbc_nopad_ctx *ctx)
{
	TEEC_UUID uuid = TA_SM4_CBC_NOPAD_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct sm4_cbc_nopad_ctx *ctx)
{
	generate_key(ctx);
	generate_iv(ctx);
	encrypt(ctx);
...
static void terminate_tee_session(struct sm4_cbc_nopad_ctx *ctx)
{
 ...
 * scp sm4_cbc_nopad/ta/d0519cfc-1839-47b8-9214-7cf6588be499.ta wenshuyu@192.168.1.6:/lib/optee_armtz
 * scp sm4_cbc_nopad/host/sm4_cbc_nopad wenshuyu@192.168.1.6:/usr/bin
 */
