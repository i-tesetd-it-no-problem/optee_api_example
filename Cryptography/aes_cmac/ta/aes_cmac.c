...
static TEE_Result generate_key(void *sess_ctx, uint32_t param_type, TEE_Param params[4])
{
    ...
    struct aes_cmac *ctx = (struct aes_cmac *)sess_ctx;
    ...
    res = TEE_IsAlgorithmSupported(TEE_ALG_AES_CMAC, TEE_CRYPTO_ELEMENT_NONE);
    if (res != TEE_SUCCESS) {
    ...
    res = TEE_AllocateOperation(&ctx->operation, TEE_ALG_AES_CMAC, TEE_MODE_MAC, KEY_BITS);
    if (res != TEE_SUCCESS) {
...
TEE_Result TA_OpenSessionEntryPoint(uint32_t param_type, TEE_Param params[4], void **sess_ctx)
{
    ...
    struct aes_cmac *ctx = TEE_Malloc(sizeof(struct aes_cmac), TEE_MALLOC_FILL_ZERO);
    if (!ctx) {
