...
static void generate_key(struct aes_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTR_GEN_KEY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void generate_iv(struct aes_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTR_GEN_IV, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void encrypt(struct aes_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTR_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct aes_ctr_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTR_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void prepare_tee_session(struct aes_ctr_ctx *ctx)
{
	TEEC_UUID uuid = TA_AES_CTR_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct aes_ctr_ctx *ctx)
{
	generate_key(ctx);
	generate_iv(ctx);
	encrypt(ctx);
...
static void terminate_tee_session(struct aes_ctr_ctx *ctx)
{
 ...
 * scp aes_ctr/ta/e2366aab-1c0a-412a-9b81-53f9d3821fc1.ta wenshuyu@192.168.1.6:/lib/optee_armtz
 * scp aes_ctr/host/aes_ctr wenshuyu@192.168.1.6:/usr/bin
 */
