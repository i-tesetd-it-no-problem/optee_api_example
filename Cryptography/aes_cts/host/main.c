...
static void generate_key(struct aes_cts_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTS_GEN_KEY, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void generate_iv(struct aes_cts_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTS_GEN_IV, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void encrypt(struct aes_cts_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTS_ENCRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void decrypt(struct aes_cts_ctx *ctx)
{
	TEEC_Result res;
	TEEC_Operation op;
	uint32_t err_origin;
	...
	res = TEEC_InvokeCommand(&ctx->sess, TA_AES_CTS_DECRYPT, &op, &err_origin);
	if(res != TEEC_SUCCESS) {
...
static void prepare_tee_session(struct aes_cts_ctx *ctx)
{
	TEEC_UUID uuid = TA_AES_CTS_UUID;
	uint32_t origin;
	TEEC_Result res;
...
static void example(struct aes_cts_ctx *ctx)
{
	generate_key(ctx);
	generate_iv(ctx);
	encrypt(ctx);
...
static void terminate_tee_session(struct aes_cts_ctx *ctx)
{
 ...
 * scp aes_cts/ta/bc78d31e-c9c9-4312-a93c-e37c9c8c7b17.ta wenshuyu@192.168.1.6:/lib/optee_armtz
 * scp aes_cts/host/aes_cts wenshuyu@192.168.1.6:/usr/bin
 */
